    ...
    class Program
    {
        ...
        private void ToMarkdown(StringWriter sw, XElement root)
        {
            ...
            else if (root.Name == "member")
            {
                ...
                if (memberType == 'T')
                {
                    int typeNameStartIndex = 3 + context["assembly"].Length;
                    string shortMemberName = memberName.Substring(typeNameStartIndex);
                    sw.WriteLine("\n## {0}\n", shortMemberName);
                    context["typeName"] = shortMemberName;
                }
                else
                {
                    int memberNameStartIndex = 
                        4 + context["assembly"].Length + context["typeName"].Length;
                    string shortMemberName = memberName.Substring(memberNameStartIndex);
